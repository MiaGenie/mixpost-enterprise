...
class TwitterPosts extends FeatureLimitResource
{
    public string $name = 'Twitter Posts';
    public string $description = 'The number of Twitter posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->whereRelation('accounts', 'provider', 'twitter')
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_facebook_twitter_posts', ['value' => $value]));
    }
}
