...
class TiktokPosts extends FeatureLimitResource
{
    public string $name = 'TikTok Posts';
    public string $description = 'The number of TikTok posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->whereRelation('accounts', 'provider', 'tiktok')
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_tiktok_daily_posts', ['value' => $value]));
    }
}
