...
class YoutubePosts extends FeatureLimitResource
{
    public string $name = 'YouTube Posts';
    public string $description = 'The number of YouTube posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->whereRelation('accounts', 'provider', 'youtube')
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_youtube_daily_posts', ['value' => $value]));
    }
}
