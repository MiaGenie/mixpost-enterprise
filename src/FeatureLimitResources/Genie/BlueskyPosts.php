...
class BlueskyPosts extends FeatureLimitResource
{
    public string $name = 'BlueSky Posts';
    public string $description = 'The number of BlueSky posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->whereRelation('accounts', 'provider', 'bluesky')
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_bluesky_daily_posts', ['value' => $value]));
    }
}
