...
class MastodonPosts extends FeatureLimitResource
{
    public string $name = 'Mastodon Posts';
    public string $description = 'The number of Mastodon posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->whereRelation('accounts', 'provider', 'mastodon')
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_mastodon_daily_posts', ['value' => $value]));
    }
}
