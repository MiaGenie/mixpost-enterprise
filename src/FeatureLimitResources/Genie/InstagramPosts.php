...
class InstagramPosts extends FeatureLimitResource
{
    public string $name = 'Instagram Posts';
    public string $description = 'The number of Instagram posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->whereRelation('accounts', 'provider', 'instagram')
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_facebook_instagram_posts', ['value' => $value]));
    }
}
