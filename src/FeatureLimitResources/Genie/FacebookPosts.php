...
class FacebookPosts extends FeatureLimitResource
{
    public string $name = 'Facebook Posts';
    public string $description = 'The number of Facebook posts users can do per day.';
    ...
    public function validator(?object $data = null): FeatureLimitResponse
    {
        ...
        $count = Post::byWorkspace($data->workspace)
            ->where(function ($query) {
                $query->whereRelation('accounts', 'provider', 'facebook_page')
                    ->orwhereRelation('accounts', 'provider', 'facebook_group');
            })
        ...
        return $this->makeFails()
            ->withMessages(__('genie.max_facebook_daily_posts', ['value' => $value]));
    }
}
